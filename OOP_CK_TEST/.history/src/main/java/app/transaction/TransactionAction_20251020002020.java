...
public class TransactionAction {
    ...
    public TransactionAction(TransactionService service) { this.service = service; }
    ...
    public void listTxCLI(Scanner sc) {
        ...
        List<app.account.Transaction> list = service.historySorted(accountId);
        System.out.println("-- Lịch sử: " + accountName + " [" + accountId + "] --");
        if (list.isEmpty()) System.out.println("(trống)");
        else for (int i = 0; i < list.size(); i++) System.out.println((i + 1) + ") " + formatTransaction(list.get(i)));
    ...
    public void editTxCLI(Scanner sc) {
        ...
        for (int i = 0; i < list.size(); i++) System.out.println((i + 1) + ") " + formatTransaction(list.get(i)));
        ...
        app.account.Transaction selected = list.get(idx);
        service.editTransaction(accountId, selected.getId(), newType, newAmount, newDate, newCat, newNote);
    ...
    public void deleteTxCLI(Scanner sc) {
        ...
        if (idx < 0 || idx >= list.size()) throw new IllegalArgumentException("STT không hợp lệ");
        app.account.Transaction selected = list.get(idx);
        service.deleteTransaction(accountId, selected.getId());
        System.out.println(">> Đã xóa. Số dư mới: " + service.getBalance(accountId).toPlainString());
    }

    private String formatTransaction(app.account.Transaction tx) {
        LocalDate date = LocalDate.ofInstant(tx.getOccurredAt(), java.time.ZoneId.systemDefault());
        String sign = (tx.getType() == TxnType.INCOME || tx.getType() == TxnType.TRANSFER_IN) ? "+" : "-";
        StringBuilder sb = new StringBuilder();
        sb.append(date.format(java.time.format.DateTimeFormatter.ofPattern("dd/MM/yyyy")))
                .append(" | ")
                .append(sign)
                .append(tx.getAmount().toPlainString())
                .append(" | ")
                .append(tx.getType());
        if (!tx.getCategory().isEmpty()) sb.append(" | #").append(tx.getCategory());
        if (!tx.getNote().isEmpty()) sb.append(" | ").append(tx.getNote());
        return sb.toString();
    }
}
